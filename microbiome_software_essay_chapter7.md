...
Multi-locus sequence typing (MLST) assigns a bacterial isolate a sequence type based on the specific combination of alleles found at a defined set of housekeeping genes (typically seven), a scheme-based approach that predates whole-genome sequencing but has been carried forward into the WGS era specifically because it preserves compatibility with decades of previously collected typing data. The `mlst` tool automates this by scanning an assembled genome against the appropriate species-specific scheme hosted by PubMLST, avoiding the wet-lab PCR and Sanger sequencing the method originally required. Unlike SeqSero2's single-species scope above, `mlst` is designed to work across the full range of bacterial species for which a PubMLST scheme exists — a genuinely different, broader typing category this chapter had not yet covered.

**Prerequisites.** `mlst` is a Perl script (from the same author, Torsten Seemann, as Prokka and Snippy elsewhere in this series) distributed via GitHub and Conda, depending on BLAST+ for its allele-matching step and bundling a regularly updated cache of PubMLST scheme definitions.

**Popularity.** `mlst` is, per an independent benchmarking publication, one of several standard programs used for sequence-type determination from assembled genomes, commonly compared against CGE/MLST, MLSTar, and the PubMLST web tool itself — a comparison set that treats `mlst` as the de facto command-line baseline any newer, faster typing tool is expected to benchmark itself against.

**Efficiency.** `mlst` has no dedicated peer-reviewed publication of its own — like Snippy (Chapter 8) from the same author, it is cited as software directly — but an independent multi-core reimplementation benchmarked against it, FastMLST, reports typing up to 28,000 genomes in under ten minutes with a 3-fold or greater speedup over `mlst` and 100% concordance with PubMLST on uncontaminated genomes, a useful third-party data point on `mlst`'s own baseline performance even though `mlst`'s original publication never reported a runtime figure itself.

- **Citation:** Cited as software (no dedicated peer-reviewed publication); see [github.com/tseemann/mlst](https://github.com/tseemann/mlst). The underlying MLST method itself originates from: Maiden MCJ, Bygraves JA, Feil E, et al. Multilocus sequence typing: a portable approach to the identification of clones within populations of pathogenic microorganisms. *Proc Natl Acad Sci U S A.* 1998;95(6):3140–3145.
- **GitHub:** [github.com/tseemann/mlst](https://github.com/tseemann/mlst)
...
**`mlst` extends, rather than complicates, this chapter's picture of Torsten Seemann's toolkit.** Prokka (Chapter 5) has a dedicated methods paper; Snippy (Chapter 8) and now `mlst` do not, both cited as software directly. Across the three tools, a consistent authorial pattern emerges: Seemann's genome-scale, standalone tools (Prokka) get published methods papers, while his narrower, single-purpose command-line utilities (Snippy, `mlst`) are released and iterated as software without a dedicated publication, relying instead on adoption and downstream citation to establish their standing — a self-consistent citation practice across one author's body of work, rather than an inconsistency to be puzzled over tool by tool.
...
6. Seemann T. mlst: Scan contig files against PubMLST typing schemes. Cited as software (no dedicated peer-reviewed publication); see github.com/tseemann/mlst. Underlying method: Maiden MCJ, Bygraves JA, Feil E, et al. Multilocus sequence typing: a portable approach to the identification of clones within populations of pathogenic microorganisms. *Proc Natl Acad Sci U S A.* 1998;95(6):3140–3145.
7. Page AJ, Cummins CA, Hunt M, Wong VK, Reuter S, Holden MTG, Fookes M, Falush D, Keane JA, Parkhill J. Roary: rapid large-scale prokaryote pan genome analysis. *Bioinformatics.* 2015;31(22):3691–3693. PMID: 26198102. doi:10.1093/bioinformatics/btv421
8. Tonkin-Hill G, MacAlasdair N, Ruis C, Weimann A, Horesh G, Lees JA, Gladstone RA, Lo S, Beaudoin C, Floto RA, Frost SDW, Corander J, Bentley SD, Parkhill J. Producing polished prokaryotic pangenomes with the Panaroo pipeline. *Genome Biol.* 2020;21(1):180. PMID: 32698896. doi:10.1186/s13059-020-02090-4
...
| mlst | Perl | https://github.com/tseemann/mlst |
